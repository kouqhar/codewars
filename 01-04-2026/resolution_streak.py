def resolution_streak(days):
  ...
  for i in range(len(days)):
    walked, screen, pages = days[i];
    if (walked >= 10000 and screen <= 120 and pages >= 5): streak+=1;
    else: pass;

  return f"Resolution on track: {streak} day streak." if streak == len(days) else f"Resolution failed on day {streak + 1}: {streak} day streak."
